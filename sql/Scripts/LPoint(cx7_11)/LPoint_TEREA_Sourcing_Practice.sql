...
(
	...
	and
	   exists (
	       ...
	       having count(distinct y.engname) < 11  and sum(x.Pack_qty) < 61.0 -- (3) SKU 11종 미만 & 구매 팩 수량 61개 미만
   )
group by t.YYYYMM, t.id 
   having
       count(distinct b.engname) < 11 -- (3) SKU 11종 미만
...
(
	...
	from cx.fct_K7_Monthly a
		join cx.product_master b on a.product_code = b.PROD_ID  and b.CIGADEVICE = 'CIGARETTES' and b.cigatype != 'CSV'
	where a.YYYYMM between '202403' and '202406'
	and
	   exists (
	       -- (2) 직전 3개월 동안 구매이력이 있는지 확인
	       select 1
...
where id ='4FE5AF12A7CF34E59E589A98247AD4D14655ABA6EDC219BF3C8F99A73C4927B1'
		and YYYYMM BETWEEN '202207' and '202301'
group by YYYYMM;

--직전 3개월 구매 상세 이력 
select engname, productdescription, prod_id, de_dt, id, gender, age, YYYYMM,x.seq, x.buy_ct, pack_qty
from cx.fct_K7_Monthly x
	join cx.product_master y on x.product_code = y.PROD_ID and y.CIGADEVICE =  'CIGARETTES' AND  y.cigatype != 'CSV' AND 4 < LEN(x.id)
where x.YYYYMM BETWEEN '202207' and '202301'
and x.id = '4FE5AF12A7CF34E59E589A98247AD4D14655ABA6EDC219BF3C8F99A73C4927B1'
order by de_dt; 
