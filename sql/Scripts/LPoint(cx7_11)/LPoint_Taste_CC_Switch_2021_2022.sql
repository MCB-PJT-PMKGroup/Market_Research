/* 작업 시작일 2024.09.25
 * 
CC Switching 2022 vs. 2021 Regular Taste CC
CC Switching 2022 vs. 2021 NTD Taste CC
CC Switching 2022 vs. 2021 Fresh Taste CC
*/
...
SELECT 
	...
	sum(case when left(a.YYYYMM, 4) = '2021' then a.pack_qty else 0 end) as [Out],
	sum(case when left(a.YYYYMM, 4) = '2022' then a.pack_qty else 0 end) as [In]
--into cx.agg_CC_Taste_Switch_2021_2022
FROM 
    cx.fct_K7_Monthly a
...
where 1=1
    ...
    and a.id not in (
		...
		where 1=1
		   	and left(x.YYYYMM, 4) in ('2021', '2022')
		GROUP BY 
		    y.FLAVORSEG_type3, y.cigatype, x.id
		HAVING 
		    -- 2022년, 2021년에 모두 구매함	
		    (SUM(CASE WHEN left(x.YYYYMM, 4) = '2022' THEN 1 ELSE 0 END) > 0
		    AND SUM(CASE WHEN left(x.YYYYMM, 4) = '2021' THEN 1 ELSE 0 END) > 0)
	)
GROUP BY 
     a.id ,b.FLAVORSEG_type3, b.cigatype
HAVING
    -- Out : 2021년도에는 구매했지만 2022년도에는 해당 제품을 구매하지 않아 Out
	(SUM(CASE WHEN  left(a.YYYYMM, 4) = '2021' THEN 1 ELSE 0 END) > 0
	and SUM(CASE WHEN  left(a.YYYYMM, 4) = '2022' THEN 1 ELSE 0 END) = 0
    AND EXISTS (
		-- 2022년에는 다른 제품을 구매한 사람
	    SELECT 1
	    FROM cx.fct_K7_Monthly x
	    	join cx.product_master y on x.product_code = y.PROD_ID and y.CIGADEVICE =  'CIGARETTES' AND  y.cigatype != 'CSV' 
	...
	OR
    -- In : 2021년도에는 구매하지 않고 2022년도에는 해당 제품을 구매하여 IN
    (SUM(CASE WHEN left(a.YYYYMM, 4) = '2022' THEN 1 ELSE 0 END) > 0
    AND SUM(CASE WHEN left(a.YYYYMM, 4) = '2021' THEN 1 ELSE 0 END) = 0 
    AND EXISTS (
    	-- 2021년에 다른 제품을 구매한 사람
	    SELECT 1
	    FROM cx.fct_K7_Monthly x
	    	join cx.product_master y on x.product_code = y.PROD_ID and y.CIGADEVICE =  'CIGARETTES' AND  y.cigatype != 'CSV' 
	    where a.id = x.id and left(x.YYYYMM, 4) = '2021'
	    and not (y.FLAVORSEG_type3 = b.FLAVORSEG_type3 and y.cigatype = b.cigatype)
    	)
    )
;
...
-- 데이터 검증
select FLAVORSEG_type3, count(*)
from cx.agg_CC_Taste_Switch_2021_2022 
group by FLAVORSEG_type3;
--New Taste	93904
--Regular	87373
--Fresh		46140
...
select  
	...
	b.FLAVORSEG_type3,
	b.New_TARSEGMENTAT,
	count(distinct case when left(a.YYYYMM, 4) = '2022' and t.[out] > 0 then t.id end ) as Out_Purchaser_Cnt,
	count(distinct case when left(a.YYYYMM, 4) = '2021' and t.[In] > 0 then t.id end ) as In_Purchaser_Cnt,
	'',
	'',
	'',
	sum(case 
		when left(a.YYYYMM, 4) = '2022' and t.[Out] > 0 then a.pack_qty else 0
	end )as Out_quantity,
	sum(case 
		when left(a.YYYYMM, 4) = '2021' and t.[In] > 0 then a.pack_qty else 0
	end) as In_quantity
from 
...
select
	...
	'',
	'',
	sum(case when left(a.YYYYMM, 4) = '2022' and t.[Out] > 0 then a.Pack_qty else 0 end ) as Out_Quantity,
	sum(case when left(a.YYYYMM, 4) = '2021' and t.[In] > 0 then a.Pack_qty else 0 end ) as In_Quantity
from 
	cx.agg_CC_Taste_Switch_2021_2022  t
