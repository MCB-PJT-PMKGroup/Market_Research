...
WITH HEETS_Purchasers AS (
	-- 1년 내에 HEETS 구매한 적이 있는 사람들 추출 (테리어 구매한 사람은 제외)
    SELECT 
        a.id,
        -- 마지막 구매일자
...
WITH HEETS_Purchasers AS (
	-- 1년 내에 HEETS 구매한 적이 있는 사람들 구매 마지막달 추출 (테리어 구매한 사람은 제외)
    SELECT 
        a.id,
        MAX(a.YYYYMM) AS LastPurchaseMonth
    FROM cx.fct_K7_Monthly a
...
;
 ...
 SELECT 
    ...
    WHERE b.ENGNAME != 'Cleaning Stick' AND b.cigatype != 'CSV'
      AND LEN(a.id) > 4
    group by a.id, ProductSubFamilyCode
    having count(ProductSubFamilyCode) =1
    and ProductSubFamilyCode ='HEETS';
...
from cx.fct_K7_Monthly a 
	JOIN cx.product_master_temp b ON a.product_code = b.PROD_ID
where a.id ='007BEBECD084A6CD3439890FA45D1AA51DF8370E00E67E14A1E04A5E7551094C'
order by a.YYYYMM;
